		...
		*/
		function barColor(index, total) {
			...
			const c = [
				...
				215,
				222
			].map((v, i) => Math.round(v + (deep[i] - v) * t));
			...
			*/
			const onListMouseMove = (e) => {
				...
				if (i >= 0 && i < marks.length && Math.abs(contentY - i * BAR_STEP) <= ADHERE_PX) {
					...
					setHovered((prev) => prev?.key === mark.key && Math.abs(prev.y - viewY) < 1 ? prev : {
						key: mark.key,
						x: rect.left + BAR_WIDTH / 2,
						y: viewY
					});
				} else if (hovered !== null) setHovered(null);
			};
			const hoveredMark = hovered === null ? void 0 : marks.find((m) => m.key === hovered.key);
